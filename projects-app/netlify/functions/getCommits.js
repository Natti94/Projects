exports.handler = async (event) => {
  ...
  const allRepos = String(qs.all_repos).toLowerCase() === "true" || String(repo).toLowerCase() === "all";
  const reposLimit = qs.repos_limit ? Math.max(1, Math.min(parseInt(qs.repos_limit, 10), 200)) : 30; // cap repos scanned
  const limit = qs.limit ? Math.max(1, Math.min(parseInt(qs.limit, 10), 5000)) : undefined; // safety cap
  ...
  try {
    ...
    // Helper to list repos for an owner (user or org) with pagination and cap
    const listRepos = async () => {
      const results = [];
      let rp = 1;
      const listEndpoint = async (base) => {
        while (results.length < reposLimit) {
          const url = `${base}?per_page=${Math.min(100, reposLimit)}&page=${rp}&type=owner&sort=updated`;
      ...
      // Try users first, then orgs as fallback
      const okUser = await listEndpoint(`https://api.github.com/users/${owner}/repos`);
      if (!okUser) {
    ...
    if (allRepos) {
      ...
      // Fetch recent commits from each repo (first page only to keep light)
      ...
      // Single repo mode (with optional pagination over all pages)
      let allData = [];
      if (fetchAll) {
        let p = page;
        while (true) {
          ...
          p += 1;
          if (!limit && allData.length >= 1000) break; // default hard cap to keep function fast
        }
      } else {
